...
public class AboutModelAssembler implements RepresentationModelAssembler<AboutDoctor, EntityModel<AboutDoctor>> {
    ...
    public EntityModel<AboutDoctor> toModel(AboutDoctor aboutDoctor) {
        ...
        EntityModel<AboutDoctor> orderModel = EntityModel.of(aboutDoctor,
        ...
        if (aboutDoctor.getSalary() == Salary.LOW) {
            orderModel.add(linkTo(methodOn(AboutController.class).upgrade(aboutDoctor.getId())).withRel("cancel"));
        }
